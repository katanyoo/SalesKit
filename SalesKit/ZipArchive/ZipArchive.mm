...
-(BOOL) UnzipOpenFile:(NSString*) zipFile
{
	...
	if( _unzFile )
	{
        _filename = [zipFile lastPathComponent];
...
-(BOOL) UnzipFileTo:(NSString*) path overWrite:(BOOL) overwrite
{
    ...
    //============= edit by Katanyoo =============
	if (success) {
		if ([_delegate respondsToSelector:@selector(zipArchive:disFinishUnzipFile:)]) {
			[_delegate zipArchive:self 
               disFinishUnzipFile:[path stringByAppendingPathComponent:_filename]];
		}
	}
	else {
		if ([_delegate respondsToSelector:@selector(zipArchive:disFailUnzipFile:)]) {
			[_delegate zipArchive:self 
                 disFailUnzipFile:[path stringByAppendingPathComponent:_filename]];
		}
	}
	//============= end edit by Katanyoo =============
